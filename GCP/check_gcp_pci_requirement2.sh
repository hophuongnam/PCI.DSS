...
default_sas=$(run_gcp_command_across_projects "gcloud iam service-accounts list" "--format=value(email)" | grep -E "(compute@developer|appspot)" | wc -l)
...
instances_with_default_sa=$(run_gcp_command_across_projects "gcloud compute instances list" "--format=value(name,serviceAccounts.email)" | grep -E "(compute@developer|appspot)" | wc -l)
...
mixed_instances=$(run_gcp_command_across_projects "gcloud compute instances list" "--format=value(name,tags.items)" | grep -i -E "(web.*db|db.*web|app.*db|db.*app)" | wc -l)
...
vpcs=$(run_gcp_command_across_projects "gcloud compute networks list" "--format=value(name)" | grep -v "^$" | wc -l)
...
external_ip_instances=$(run_gcp_command_across_projects "gcloud compute instances list" "--format=value(name,networkInterfaces[0].accessConfigs[0].natIP)" | grep -v "None" | grep -v "^$" | wc -l)
...
permissive_fw_rules=$(run_gcp_command_across_projects "gcloud compute firewall-rules list" "--format=value(name,sourceRanges.join(','),allowed[].map().firewall_rule().list():label=ALLOW,targetTags.join(','),network)")
...
cloud_sql_instances=$(run_gcp_command_across_projects "gcloud sql instances list" "--format=value(name)")
...
insecure_protocols=$(run_gcp_command_across_projects "gcloud compute firewall-rules list" "--format=value(name,allowed)" | grep -E "(tcp:21|tcp:23|tcp:53|udp:69)" | wc -l)
...
ssh_password_instances=$(run_gcp_command_across_projects "gcloud compute instances list" "--format=value(name,metadata.items)" | grep -i "enable-oslogin.*false" | wc -l)
...
buckets=$(run_gcp_command_across_projects "gsutil ls" "" 2>/dev/null | grep "gs://" | sed 's|gs://||' | sed 's|/$||')
bucket_count=0
...
default_sa_usage=$(run_gcp_command_across_projects "gcloud compute instances list" "--format=value(name,serviceAccounts.email)" | grep -E "(compute@developer|appspot)" | wc -l)
...
load_balancers=$(run_gcp_command_across_projects "gcloud compute forwarding-rules list" "--format=value(name,target,portRange)" | grep -E "(https|ssl)")
...
if [ $lb_count -gt 0 ]; then
    ...
    http_lbs=$(run_gcp_command_across_projects "gcloud compute forwarding-rules list" "--format=value(name,target,portRange)" | grep -E "80|8080")
...
all_fw_rules=$(run_gcp_command_across_projects "gcloud compute firewall-rules list" "--format=value(name,targetTags.join(','),targetServiceAccounts.join(','),network)")
unused_fw_rules=0
    ...
    while IFS=$'\t' read -r fw_name tags service_accounts network; do
        ...
        if [ -n "$tags" ]; then
            ...
            for tag in "${TAG_ARRAY[@]}"; do
                instances_with_tag=$(run_gcp_command_across_projects "gcloud compute instances list" "--format=value(name)" "--filter=tags.items:$tag" | wc -l)
...
unused_static_ips=$(run_gcp_command_across_projects "gcloud compute addresses list" "--format=value(name,status)" | grep "RESERVED" | wc -l)
