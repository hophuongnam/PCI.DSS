...
"""
PCI DSS 4.0 Interactive Summary Report Generator - Fixed Version
This script creates a properly structured interactive summary report with all requirements at the same level.
"""
...
def main():
    print("Generating fixed PCI DSS 4.0 interactive summary report...")
    
    # Output file
    summary_report = "pci_dss_fixed_summary.html"
    ...
    html_content = generate_fixed_html(requirements_data, overall_stats)
    
    # Write the final HTML to file
    with open(summary_report, "w") as f:
        f.write(html_content)
    
    print(f"Fixed interactive summary report created successfully: {summary_report}")
...
def generate_fixed_html(requirements_data, overall_stats):
    """Generate fixed interactive HTML with proper hierarchy"""
    ...
    # Generate requirements table rows with expandable sections - with fixed structure
    req_rows = []
    for req in requirements_data:
        ...
        # Create a complete row with all cells
        row = f"""
        <tr>
    ...
    # Generate critical findings section with expandable details - with fixed structure
